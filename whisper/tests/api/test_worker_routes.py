...
class FakeScheduler:
    def __init__(self, job: Job, worker_id: str = "worker-1") -> None:
        self.job: Job = job
...
def job() -> TranscriptionJob:
    return TranscriptionJob(
        params=TranscriptionParams(
            teletask_id=12345, mp4_url="http://example.com/video.mp4"
        )
    )
...
@pytest.fixture
def scrape_job() -> ScrapeLectureDataJob:
    return ScrapeLectureDataJob(params=ScrapeLectureDataParams(teletask_id=12345))
...
class TestWorkerRoutesV2:
    ...
    def test_scrape_result_v2_rejects_flat_lecture_payload(
        ...
        assert scrape_job.status == "ENQUEUED"
