...
const server: FastifyInstance<RawServerDefault> = fastify<RawServerDefault>({
  trustProxy: true,
  logger: {
    level: 'info',
    transport: {
  ...
  if (!authHeader) {
    ...
    if (!token) {
      ...
      if (!apiKey) {
        ...
        return reply.code(403).send({ error: 'API key has been revoked' });
      } else if (apiKey.status === 'expired') {
        return reply.code(403).send({ error: 'API key has expired' });
      } else if (apiKey.status !== 'active') {
        return reply.code(403).send({ error: 'API key is not active' });
      } else {
        const now = new Date();
...
server.get<{ Params: SubRouteParams }>('/sub/:id/:language', { preHandler: verifyAuthHeader }, async (
  ...
  if (Number.isNaN(teletaskId) || teletaskId <= 0 || teletaskId.toString() !== id) {
...
server.listen({ port: 80, host: '0.0.0.0' }, (err: Error | null, address: string) => {
  if (err) {
    console.error(err);
    process.exit(1);
  }
